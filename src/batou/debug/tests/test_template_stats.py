...
def test_template_stats_humanize_empty():
    """Test humanize returns empty string for no stats."""
    stats = TemplateStats()
    result = stats.humanize()
    assert result == ""


def test_template_stats_humanize_with_data():
    """Test humanize formats stats correctly."""
    stats = TemplateStats()
    stats.record_hit(80)
    ...
    result = stats.humanize()
    assert "80 hits" in result
    assert "20 misses" in result
    assert "100 cached templates" in result
    assert "80.0% hit rate" in result


def test_template_stats_humanize_hit_rate():
    """Test humanize calculates hit rate correctly."""
    stats = TemplateStats()
    stats.record_hit(9)
    stats.record_miss(1)

    result = stats.humanize()
    assert "90.0% hit rate" in result


def test_template_stats_reset():
